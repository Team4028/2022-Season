...
public class EvacuateWrongCargo extends CommandBase {
    ...
    public void initialize() {
        if (ColorSensor.getInstance().getInnerBallCorrect() && ColorSensor.getInstance().getOuterBallCorrect()) {
            ethird.schedule();
        } else if (!ColorSensor.getInstance().getInnerBallCorrect() && !ColorSensor.getInstance().getOuterBallCorrect()
                && ColorSensor.getInstance().getInnerFilled() && ColorSensor.getInstance().getOuterFilled()) {
            eouter.schedule();
            einner.schedule();
        } else if (ColorSensor.getInstance().getInnerBallCorrect() && !ColorSensor.getInstance().getOuterBallCorrect()
                && ColorSensor.getInstance().getOuterFilled()) {
            eouter.schedule();
        } else if (!ColorSensor.getInstance().getInnerBallCorrect() && ColorSensor.getInstance().getOuterBallCorrect()
                && ColorSensor.getInstance().getInnerFilled()) {
            einner.schedule();
        }
